...
<form
    ...
    method="post"
    action=""
    onSubmit="autogroupObj.selectAll(); autouserObj.selectAll(); autoadminObj.selectAll(); return validateForm();"
>
...
$table->add(new TrFormElement(_T("Description", "samba4"), new InputTpl("shareDescription")), 
        array("value" => $shareDescription));
$table->display();
?>
...
<input
        ...
        type="submit"
        class="btnPrimary"
        value="<?php echo  _T("Create", "samba4"); ?>"
    />
<?php } else { ?>
<input
        name="share"
        type="hidden"
        value="<?php echo $share; ?>"
    /> <input
        name="bshareedit"
        type="submit"
        class="btnPrimary"
        value="<?php echo  _T("Confirm", "samba4"); ?>"
    />
<?php
...
function _parseForm() {
    ...
    // TODO: understand why $FH->getPostValue($_POST["groupgroupsselected"]) does not work
    // $shareGroup = $FH->getPostValue($_POST["groupgroupsselected"]);
    $shareGroup = isset($_POST["groupgroupsselected"]) ? $_POST["groupgroupsselected"] : [];
    // $shareUser = $FH->getPostValue($_POST["userusersselected"]);
    $shareUser = isset($_POST["userusersselected"]) ? $_POST["userusersselected"] : [];
    ...
    return array($shareName, $sharePath, $shareDescription, $shareEnabled, $shareGuest, $shareGroup, 
            $shareUser);
}
